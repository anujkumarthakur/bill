          ...
          <div key={dev.device_id || i}>
            ...
            >
              <div>
                ...
                <div style={styles.deviceMeta}>ID: {shortId} &middot; {dev.model || '-'} &middot; {dev.os_version || '-'} &middot; SIM: {(() => { try { const si = JSON.parse(dev.sim_info || '[]'); return si.map(s => s.number || s.carrier || '?').join(', ') } catch { return '-'; } })()} &middot; SMS: {smsList.length} &middot; Contacts: {contactsList.length}</div>
              </div>
